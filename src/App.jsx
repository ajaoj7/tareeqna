...
// مفتاح الخرائط: (اتركه فارغاً وسيعمل الموقع بدونه بشكل طبيعي)
...
const App = () => {
  ...
  // إدارة الدخول الرقمي مع "مؤقت أمان" لمنع الشاشة البيضاء
  useEffect(() => {
    // إذا لم يستجب Firebase خلال 5 ثوانٍ، افتح الموقع كضيف تلقائياً
      ...
      <div className="bg-white p-10 rounded-[3.5rem] border border-slate-100 shadow-sm space-y-8 text-right">
         <div className="space-y-4">
            ...
            <div className="relative">
              ...
              <input 
                placeholder={GOOGLE_MAPS_KEY ? "الموقع (تلقائي من الخريطة)" : "اكتب الموقع يدوياً (مثلاً: شارع وصفي التل)"} 
        ...
        {items.length > 0 ? items.map(item => (
           <div key={item.id} className="bg-white p-8 md:p-12 rounded-[4.5rem] border border-slate-100 shadow-sm space-y-10 group hover:shadow-2xl transition-all duration-700 text-right leading-none">
              <div className="flex flex-col md:flex-row gap-12 text-right">
                 <div className="relative w-full md:w-80 h-80 flex-shrink-0 overflow-hidden rounded-[3.5rem] shadow-inner"><img src={item.img} className="w-full h-full object-cover transition-transform group-hover:scale-110 duration-1000" /><div className="absolute top-6 right-6 bg-red-600 text-white text-[10px] font-black px-6 py-2 rounded-full shadow-xl ring-4 ring-white uppercase tracking-widest leading-none animate-pulse">عاجل</div></div>
...
const LoadingScreen = () => (
  <div className="h-screen flex flex-col items-center justify-center bg-[#F8FAFC] gap-6 animate-in fade-in duration-500 text-right">
    ...
    <div className="text-center animate-pulse"><p className="text-3xl font-black text-slate-800 italic leading-none">طريقنا</p><p className="text-[10px] font-bold text-slate-400 mt-2 uppercase tracking-widest leading-none">إعمار الأردن الرقمي 2030</p></div>
  </div>
);
