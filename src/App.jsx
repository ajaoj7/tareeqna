      ...
      <header className="bg-white/80 backdrop-blur-xl border-b border-slate-100 sticky top-0 z-[60] p-3 md:p-4 flex justify-between items-center shadow-sm">
        <div className="flex items-center gap-2 md:gap-3 cursor-pointer group shrink-0" onClick={() => setView('landing')}>
          ...
          <div className="flex flex-col leading-none select-none">
            {/* الشعار الموحد: تم استخدام span متداخل بدون flex لمنع الانفصال */}
        ...
        <div>
           <div className="bg-green-600 p-8 rounded-[3rem] text-white shadow-xl relative overflow-hidden group">
              <div className="relative z-10 text-right">
                 ...
                 <p className="text-xs text-green-50 leading-relaxed font-bold">جميع البلاغات تخضع لنظام التحقق لضمان توجيه الدعم لمستحقيه فعلياً.</p>
              </div>
...
// --- Camera & AI Logic (Restored Live Camera) ---
const CameraReportView = ({ onComplete, user }) => {
  ...
  // --- تشغيل الكاميرا الحية (Live Camera) ---
  const startCamera = async () => {
    ...
    try {
      ...
      if (videoRef.current) {
        ...
        videoRef.current.setAttribute("playsinline", true); // للأيفون
        videoRef.current.play();
      }
    } catch (err) {
      console.error("Camera Error", err);
      alert("يرجى السماح بالوصول للكاميرا أو استخدام خيار الملفات.");
  ...
  const capturePhoto = () => {
    ...
    if (video && canvas) {
       canvas.width = 800; // تصغير الحجم فوراً
       ...
       setImg(canvas.toDataURL('image/jpeg', 0.6)); // جودة مضغوطة
       setIsCameraOpen(false);
    }
  };
  ...
  const handleSmartSubmission = async () => {
    ...
    if (!img) return setError("يجب تصوير الضرر ميدانياً.");
    
    setSubmitting(true); setError(null);
    const submissionTimeout = setTimeout(() => {
        if (submitting) { setSubmitting(false); setError("بطء في الاتصال. يرجى المحاولة."); }
    }, 20000);

    try {
      if (!db) throw new Error("No DB");
      // تحليل وهمي للبيانات لسرعة الرد
      const streetType = location.includes("مطار") ? "سيادي" : "حيوي";
      
      await addDoc(collection(db, 'artifacts', appId, 'public', 'data', 'reports'), {
        userId: user?.uid || "guest", img, location, streetType, severity: 8,
        goal: 400, pitDepth: "12.5", collected: 0, createdAt: new Date().toISOString(), status: 'verified_ai'
      });
      clearTimeout(submissionTimeout);
      onComplete();
    } catch (e) {
      clearTimeout(submissionTimeout);
      setError("خطأ في الإرسال. تحقق من الإنترنت.");
      setSubmitting(false);
    }
  };
  ...
  if (isCameraOpen) {
    return (
      <div className="fixed inset-0 bg-black z-[100] flex flex-col">
        ...
        <div className="p-10 bg-black/80 flex justify-center gap-10">
           <button onClick={() => { setIsCameraOpen(false); const s=videoRef.current?.srcObject; if(s) s.getTracks().forEach(t=>t.stop()); }} className="text-white font-bold p-4">إلغاء</button>
           <button onClick={capturePhoto} className="w-20 h-20 bg-white rounded-full border-4 border-slate-300 active:scale-90 transition-transform"></button>
  ...
  return (
    <div className="p-4 md:p-10 space-y-8 animate-in slide-in-from-bottom-12 max-w-4xl mx-auto text-right leading-none">
      ...
      {isPC && !img ? (
        <div className="flex flex-col md:flex-row gap-8 items-center bg-amber-50 p-8 md:p-12 rounded-[2.5rem] border-2 border-amber-100 animate-in zoom-in leading-relaxed text-right">
           <div className="flex-1 space-y-6">
              ...
              <p className="text-amber-800 font-bold text-base md:text-lg max-w-md mx-auto leading-relaxed italic text-right">لضمان دقة التوثيق، يفضل استخدام الهاتف.</p>
           </div>
           <div className="bg-white p-6 rounded-[2rem] shadow-xl border border-amber-200 flex flex-col items-center shrink-0">
              ...
              <p className="text-[10px] font-black text-slate-400 mt-4 uppercase tracking-widest text-center leading-none text-right">امسح الكود</p>
           </div>
        </div>
      ) : (
        <div className="bg-white p-8 md:p-10 rounded-[3rem] border-4 border-dashed border-slate-200 flex flex-col items-center justify-center min-h-[350px] relative overflow-hidden cursor-pointer group hover:border-green-500 shadow-inner text-center transition-all leading-none" onClick={startCamera}>
          {img ? <img src={img} className="absolute inset-0 w-full h-full object-cover leading-none" alt="Captured" /> : 
            <div className="text-center space-y-6 animate-in zoom-in leading-none">
               ...
               <div className="leading-none"><p className="text-2xl font-black text-slate-700 leading-none italic uppercase text-center leading-none">فتح الكاميرا</p><p className="text-sm font-bold text-slate-400 mt-4 max-w-xs mx-auto italic leading-none text-center leading-none">اضغط هنا للالتقاط المباشر</p></div>
            </div>
          }
        </div>
      )}
      ...
      <button onClick={handleSmartSubmission} disabled={submitting} className="w-full bg-slate-900 text-white py-6 md:py-8 rounded-[2rem] font-black text-2xl md:text-4xl shadow-2xl active:scale-95 disabled:bg-slate-400 transition-all flex items-center justify-center gap-4 group leading-none text-center">
        {submitting ? <Loader2 className="animate-spin" size={32}/> : <>تحقق ونشر البلاغ <HardHat size={28} className="rotate-12 group-hover:rotate-0 transition-transform leading-none"/></>}
      </button>
      <canvas ref={canvasRef} className="hidden" />
    </div>
  );
  ...
  <div className="bg-white p-6 md:p-12 rounded-[2.5rem] md:rounded-[4.5rem] border border-slate-100 shadow-sm space-y-8 group hover:shadow-2xl transition-all duration-700 text-right leading-none text-right text-right text-right">
     <div className="flex flex-col lg:flex-row gap-8 md:gap-12 leading-none text-right text-right text-right text-right">
        ...
        <div className="flex-1 space-y-6 md:space-y-8 text-right flex flex-col justify-center leading-none text-right text-right text-right text-right text-right">
           ...
           <div className="flex flex-wrap gap-2 justify-end leading-none text-right text-right text-right text-right text-right text-right"><span className="bg-slate-100 text-slate-600 px-3 py-1 rounded-full text-[10px] font-black italic text-right leading-none text-right text-right text-right text-right">{item.streetType}</span><span className="bg-amber-100 text-amber-700 px-3 py-1 rounded-full text-[10px] font-black italic text-right leading-none text-right text-right text-right text-right">عمق {item.pitDepth}سم</span></div>
  ...
  <div className="fixed inset-0 bg-slate-900/95 backdrop-blur-3xl z-[100] flex items-center justify-center p-4 md:p-6 animate-in fade-in leading-none text-right text-right">
    <div className="bg-white rounded-[3rem] md:rounded-[6rem] w-full max-w-4xl p-8 md:p-20 space-y-10 shadow-2xl relative overflow-hidden animate-in zoom-in ring-1 ring-slate-100 text-right leading-none text-right text-right text-right">
      ...
      <div className="bg-slate-50 p-8 md:p-12 rounded-[2.5rem] md:rounded-[4.5rem] border border-slate-100 shadow-inner space-y-8 text-right leading-relaxed text-right text-right text-right text-right">
         ...
         <p className="text-slate-600 text-xl md:text-3xl leading-[1.6] font-medium tracking-tight text-right leading-relaxed italic text-right text-right text-right text-right text-right">يخضع هذا الموقع لرقابة مهندسي المساحة والتعبيد في أمانة عمان الكبرى لضمان جودة التعبيد لأكثر من 15 عاماً.</p>
      </div>
    </div>
  </div>
);
