      ...
      <header className="bg-white/80 backdrop-blur-xl border-b border-slate-100 sticky top-0 z-[60] p-3 md:p-4 flex justify-between items-center shadow-sm">
        <div className="flex items-center gap-2 md:gap-3 cursor-pointer group shrink-0" onClick={() => setView('landing')}>
          ...
          <div className="leading-none flex flex-col">
            {/* تم حل مشكلة انقطاع الشعار بشكل نهائي */}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-12 text-right">
        ...
        <div className="space-y-6">
          ...
          <ul className="space-y-3">
            {['لوحة التحكم', 'بلاغاتي', 'سجل مساهماتي', 'الملف الشخصي'].map((item, i) => (
              <li key={i} className="group flex items-center gap-2 cursor-pointer justify-start">
        ...
        <div className="space-y-6">
          ...
          <ul className="space-y-3 text-right">
            <li onClick={() => setView('partner-portal')} className="flex items-center gap-2 cursor-pointer justify-start"><div className="w-1.5 h-1.5 bg-slate-200 rounded-full"></div><span className="text-sm font-black text-slate-500">عن طريقنا</span></li>
            <li className="flex items-center gap-2 cursor-pointer justify-start"><div className="w-1.5 h-1.5 bg-slate-200 rounded-full"></div><span className="text-sm font-black text-slate-500">سياسة الخصوصية</span></li>
          </ul>
        </div>
        <div>
           <div className="bg-green-600 p-8 rounded-[3rem] text-white shadow-xl relative overflow-hidden group">
              <div className="relative z-10 text-right">
                 ...
                 <p className="text-xs text-green-50 leading-relaxed font-bold">جميع البلاغات تخضع لنظام التحقق لضمان توجيه الدعم لمستحقيه.</p>
              </div>
           </div>
        </div>
      </div>
        ...
        <aside className="w-full lg:w-72 space-y-6 order-2 lg:order-1">
           <div className="bg-white p-6 md:p-8 rounded-[2.5rem] md:rounded-[3rem] border border-slate-100 shadow-sm sticky top-28">
              <div className="space-y-1 mb-8 border-r-4 border-green-600 pr-4 text-right">
                 <h3 className="text-xl font-black text-slate-800">شركاء الإعمار</h3>
                 <p className="text-[10px] font-bold text-slate-400 tracking-wider italic">Corporate Partners</p>
              </div>
              <div className="space-y-6">
                 {sponsors.map(s => (
                   <div key={s.id} className="flex items-center gap-4 group cursor-pointer text-right transition-transform hover:-translate-x-1 justify-start">
                      <div className="w-10 h-10 md:w-12 md:h-12 rounded-2xl bg-slate-50 flex items-center justify-center p-2 border border-slate-50 transition-all"><img src={s.logo} className="w-full h-full object-contain grayscale group-hover:grayscale-0" alt={s.name} /></div>
                      <div className="flex-1 min-w-0"><p className="font-black text-xs md:text-sm text-slate-800 truncate mb-1">{s.name}</p><p className="text-[9px] md:text-[10px] font-bold text-green-600 leading-none flex items-center gap-1"><CheckCircle2 size={10}/> {s.repairs} مشروع منجز</p></div>
                   </div>
                 ))}
              </div>
        ...
        <div className="flex-1 space-y-12 md:space-y-20 order-1 lg:order-2">
          <section className="bg-slate-900 rounded-[2.5rem] md:rounded-[3.5rem] p-8 md:p-16 text-white relative overflow-hidden shadow-2xl border border-white/5 text-right">
            <div className="relative z-10 max-w-2xl space-y-6 text-right leading-none">
              ...
              <p className="text-slate-400 text-sm md:text-xl font-medium leading-relaxed max-w-lg italic text-right">المنصة الذكية الموثقة لإصلاح وتجميل البنية التحتية بالتعاون مع القطاع الخاص.</p>
...
const HomeSection = ({ title, items, setView, icon, isSuccess }) => (
  <div className="space-y-6 px-2 text-right">
    <div className="flex justify-between items-center border-r-4 border-slate-900 pr-4 leading-none text-right">
       <div className="flex items-center gap-3 justify-start text-right leading-none">
          <div className="shrink-0">{icon}</div>
          <h3 className="text-lg md:text-2xl font-black text-slate-800 tracking-tight leading-none text-right">{title}</h3>
       </div>
    ...
    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 md:gap-6">
      {items.length > 0 ? items.map(item => (
        <div key={item.id} onClick={() => setView('roads')} className="bg-white p-4 rounded-[2rem] border border-slate-100 shadow-sm hover:shadow-xl transition-all cursor-pointer group relative overflow-hidden flex flex-col h-full text-right leading-none">
           <div className="relative h-40 md:h-48 overflow-hidden rounded-2xl mb-4"><img src={item.img} className="w-full h-full object-cover transition-transform duration-700 group-hover:scale-110" alt="Repair" />{isSuccess && <div className="absolute inset-0 bg-green-600/30 backdrop-blur-[2px] flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity"><div className="bg-white text-green-600 p-3 rounded-full shadow-2xl"><CheckCircle2 size={32}/></div></div>}</div>
           <div className="space-y-2 flex-1 text-right leading-none"><h4 className="font-black text-slate-800 text-sm md:text-lg truncate mb-2 text-right">{item.location}</h4><div className="flex items-center gap-1 text-slate-400 font-bold text-[10px] md:text-xs justify-end leading-none">{item.streetType || "شارع فرعي"} <MapPin size={12} className="text-green-600"/></div></div>
           <div className="pt-4 border-t mt-3"><div className="w-full bg-slate-100 h-1.5 rounded-full overflow-hidden leading-none"><div className={`${isSuccess ? 'bg-green-500' : 'bg-amber-500'} h-full rounded-full transition-all`} style={{width: `${Math.min(((item.collected||0)/(item.goal||100))*100, 100)}%`}}></div></div></div>
        </div>
      )) : <p className="col-span-full text-center py-10 text-slate-300 font-bold italic">بانتظار تحديث البيانات الميدانية...</p>}
    </div>
  </div>
);
...
const CameraReportView = ({ onComplete, user }) => {
  ...
  const jordanMegaAI = (text) => {
    ...
    const dbKeys = {
      ...
      commercial: ["جاردنز", "جامعة", "رينبو", "عبدلي", "سويفية", "خلدا", "مكة", "مدينة", "وصفي", "تلاع", "صويفية", "رابية", "شميساني"],
      landmarks: ["مول", "تاج", "سيتي", "مجمع", "مستشفى", "حدائق", "جامع", "كنيسة", "سياح", "اثر", "قلعة", "مدرج", "بوليفارد"],
    };
    if (dbKeys.sovereign.some(k => txt.includes(k))) return { type: "طريق سيادي / دولي", severity: 9, goal: 550, importance: "سيادي" };
    if (dbKeys.commercial.some(k => txt.includes(k))) return { type: "منطقة تجارية حيوية", severity: 8, goal: 400, importance: "اقتصادي" };
    if (dbKeys.landmarks.some(k => txt.includes(k))) return { type: "منطقة معالم وازدحام", severity: 7, goal: 350, importance: "سياحي" };
  ...
  const handleSmartSubmission = async () => {
    ...
    // محاكاة تحليل صورة وعمق
    setTimeout(async () => {
      const evalData = jordanMegaAI(location);
      try {
        await addDoc(collection(db, 'artifacts', appId, 'public', 'data', 'reports'), {
          userId: user?.uid || "guest", img, location, streetType: evalData.type, severity: evalData.severity,
          goal: evalData.goal, pitDepth: (Math.random() * 10 + 5).toFixed(1), importance: evalData.importance,
  ...
  return (
    <div className="p-4 md:p-10 space-y-8 animate-in slide-in-from-bottom-12 max-w-4xl mx-auto text-right leading-none">
      <div className="flex items-center justify-between"><h2 className="text-3xl font-black text-slate-800 tracking-tighter leading-none text-right">بلاغ ميداني ذكي</h2><button onClick={onComplete} className="p-2 md:p-3 bg-white rounded-full text-slate-400 border border-slate-100 shadow-sm leading-none"><X/></button></div>
      {isPC && !img ? (
        <div className="flex flex-col md:flex-row gap-8 items-center bg-amber-50 p-8 md:p-12 rounded-[2.5rem] border-2 border-amber-100 animate-in zoom-in leading-relaxed text-right">
           <div className="flex-1 space-y-6">
              <Smartphone size={60} className="text-amber-600" />
              <h3 className="text-2xl font-black text-amber-900 leading-none uppercase text-right">انتقل لهاتفك المحمول</h3>
              <p className="text-amber-800 font-bold text-base leading-relaxed italic text-right">لضمان دقة التوثيق الميداني وتحديد عمق الضرر آلياً، نوصي بفتح المنصة عبر هاتفك المحمول واستخدام الكاميرا المباشرة.</p>
           ...
           <div className="bg-white p-6 rounded-[2rem] shadow-xl border border-amber-200 flex flex-col items-center shrink-0">
              <img src="https://api.qrserver.com/v1/create-qr-code/?size=150x150&data=https://tareeqna.vercel.app/" className="w-32 h-32 md:w-40 md:h-40" alt="QR" />
        ...
        <div className="bg-white p-8 md:p-10 rounded-[3rem] border-4 border-dashed border-slate-200 flex flex-col items-center justify-center min-h-[350px] relative overflow-hidden cursor-pointer group hover:border-green-500 shadow-inner text-center transition-all leading-none" onClick={() => fileInputRef.current.click()}>
          {img ? <img src={img} className="absolute inset-0 w-full h-full object-cover leading-none" alt="Verification" /> : 
            <div className="text-center space-y-6 animate-in zoom-in leading-none">
               ...
               <div className="leading-none"><p className="text-2xl font-black text-slate-700 leading-none italic uppercase text-center leading-none">إضغط للتصوير الميداني</p><p className="text-sm font-bold text-slate-400 mt-4 max-w-xs mx-auto italic leading-none text-center leading-none">سيتم تحليل الصورة وعمق الحفرة آلياً.</p></div>
            </div>
          }
      ...
      <div className="bg-white p-8 rounded-[2.5rem] border border-slate-100 shadow-sm space-y-6 relative text-right leading-none">
         <div className="space-y-4 leading-none text-right">
            <label className={`text-xs font-black uppercase tracking-widest flex items-center gap-2 leading-none justify-start text-right ${error ? 'text-red-600 animate-pulse' : 'text-slate-400'}`}>الموقع الوصفي الدقيق <ShieldCheck size={14} className={error ? 'text-red-600' : 'text-indigo-600'}/></label>
            <div className="relative text-right leading-none"><MapPin className={`absolute right-6 top-1/2 -translate-y-1/2 transition-colors leading-none ${error ? 'text-red-600' : 'text-green-600'}`} size={24}/><input placeholder="شارع المطار، سيتي مول، شارع الجاردنز..." className={`w-full p-6 md:p-8 pr-14 md:pr-16 bg-slate-50 border-4 rounded-[2rem] font-black outline-none transition-all text-xl md:text-2xl shadow-inner text-right leading-none ${error ? 'border-red-500 bg-red-50' : 'border-transparent focus:border-green-600 focus:bg-white'}`} value={location} onChange={e => { setLocation(e.target.value); if(e.target.value.trim()) setError(null); }} /></div>
            {error && <div className="flex items-start gap-3 bg-red-100 text-red-600 p-5 rounded-[1.8rem] border-2 border-red-200 animate-in slide-in-from-top-4 text-right leading-none"><AlertCircle size={24} className="shrink-0 mt-1 leading-none" /><p className="font-black text-sm md:text-lg text-right leading-none">{error}</p></div>}
         </div>
      </div>
      <button onClick={handleSmartSubmission} disabled={submitting} className="w-full bg-slate-900 text-white py-6 md:py-8 rounded-[2rem] font-black text-2xl md:text-4xl shadow-2xl active:scale-95 disabled:bg-slate-400 transition-all flex items-center justify-center gap-4 group leading-none text-center">
        {submitting ? <Loader2 className="animate-spin leading-none" size={32}/> : <>تحقق ونشر البلاغ <HardHat size={28} className="rotate-12 group-hover:rotate-0 transition-transform leading-none"/></>}
      </button>
    </div>
  );
};
...
const ListView = ({ title, items, isStadium }) => (
  <div className="py-8 md:py-12 space-y-10 animate-in slide-in-from-right duration-700 text-right leading-none">
     <div className="px-2 border-r-8 border-slate-900 pr-4 md:pr-6 text-right leading-none">
        ...
        <p className="text-slate-400 font-bold text-[10px] md:text-xs uppercase tracking-widest mt-2 leading-none text-right">Verified Infrastructure Hub</p>
     </div>
...
const DetailedCard = ({ item }) => (
  <div className="bg-white p-6 md:p-12 rounded-[2.5rem] md:rounded-[4.5rem] border border-slate-100 shadow-sm space-y-8 group hover:shadow-2xl transition-all duration-700 text-right leading-none text-right">
     <div className="flex flex-col lg:flex-row gap-8 md:gap-12 leading-none text-right text-right">
        <div className="relative w-full lg:w-80 h-64 md:h-80 flex-shrink-0 overflow-hidden rounded-[2.5rem] md:rounded-[3.5rem] shadow-inner leading-none text-right text-right">
           <img src={item.img} className="w-full h-full object-cover transition-transform group-hover:scale-110 duration-1000 leading-none text-right text-right" alt="Damage" />
        ...
        <div className="flex-1 space-y-6 md:space-y-8 text-right flex flex-col justify-center leading-none text-right text-right text-right">
           <div className="flex justify-between items-start leading-none w-full text-right text-right text-right"><h4 className="font-black text-3xl md:text-5xl text-slate-800 tracking-tighter italic text-right shrink-0 leading-none text-right text-right">إصلاح وتعبيد</h4><button className="p-3 md:p-4 bg-indigo-50 text-indigo-600 rounded-xl shadow-lg active:scale-90 hover:bg-indigo-600 hover:text-white transition-all leading-none text-right text-right"><Info size={24} className="leading-none text-right text-right"/></button></div>
           <p className="text-slate-400 font-bold text-xl md:text-2xl flex items-center gap-3 justify-end leading-none text-right italic leading-none text-right text-right"><MapPin size={24} className="text-green-600 leading-none text-right text-right"/> {item.location}</p>
           <div className="flex flex-wrap gap-2 justify-end leading-none text-right text-right text-right"><span className="bg-slate-100 text-slate-600 px-3 py-1 rounded-full text-[10px] font-black italic text-right leading-none text-right text-right">{item.streetType}</span><span className="bg-amber-100 text-amber-700 px-3 py-1 rounded-full text-[10px] font-black italic text-right leading-none text-right text-right">عمق {item.pitDepth}سم</span></div>
           <div className="pt-6 space-y-4 text-right leading-none text-right text-right text-right">
              <div className="flex justify-between text-[10px] md:text-sm font-black uppercase tracking-widest text-slate-500 leading-none text-right text-right text-right"><span className="text-right text-right text-right">المساهمات: <span className="text-slate-900 text-xl md:text-2xl leading-none text-right text-right text-right">{item.collected || 0} د.أ</span></span><span className="text-right text-right text-right">الهدف الوطني: {item.goal || 150} د.أ</span></div>
              <div className="w-full bg-slate-100 h-4 rounded-full overflow-hidden shadow-inner p-1 leading-none text-right text-right text-right"><div className="bg-gradient-to-l from-green-500 to-green-700 h-full rounded-full transition-all duration-1000 leading-none text-right text-right text-right" style={{width: `${Math.min(((item.collected||0)/(item.goal||100))*100, 100)}%`}}></div></div>
           </div>
        </div>
     </div>
  </div>
  ...
  <div className="py-8 md:py-12 space-y-12 animate-in fade-in text-right leading-none">
    <div className="bg-amber-50 p-8 md:p-16 rounded-[2.5rem] md:rounded-[4rem] border-2 border-amber-100 flex flex-col md:flex-row items-center gap-8 shadow-inner relative overflow-hidden text-right leading-none">
       <div className="space-y-4 md:space-y-6 relative z-10 text-right w-full leading-none order-2 md:order-1 text-right">
         <h2 className="text-3xl md:text-4xl font-black text-amber-900 leading-none italic uppercase text-right w-full leading-none text-right">صندوق الإعمار الرقمي</h2>
         <p className="text-amber-700 text-lg md:text-2xl font-bold opacity-80 leading-relaxed italic text-right leading-relaxed text-right">تبرعاتك تذهب مباشرة لصيانة شوارعنا وملاعبنا استعداداً للاستحقاقات الوطنية الكبرى. كل دينار يساهم في جعل وطننا أكثر أماناً وجمالاً.</p>
       </div>
    ...
    <div className="grid grid-cols-1 md:grid-cols-2 gap-8 md:gap-10 text-right leading-none text-right">
       {reports.length > 0 ? reports.map(r => (<div key={r.id} className="bg-white p-6 md:p-10 rounded-[2.5rem] md:rounded-[4rem] border border-slate-100 shadow-sm space-y-6 group hover:shadow-xl transition-all leading-none text-right flex flex-col h-full text-right"><img src={r.img} className="w-full h-56 md:h-64 rounded-[2rem] md:rounded-[3rem] object-cover shadow-xl grayscale-[0.2] group-hover:grayscale-0 transition-all duration-500 text-right" alt="Repair" /><div className="space-y-6 text-right flex-1 flex flex-col justify-between leading-none text-right"><h4 className="font-black text-2xl md:text-3xl truncate text-slate-800 italic leading-none text-right text-right">{r.location}</h4><div className="flex justify-between items-center bg-slate-50 p-6 md:p-8 rounded-[1.5rem] md:rounded-[2rem] border border-slate-100 shadow-inner text-right leading-none text-right"><p className="text-green-600 font-black text-2xl md:text-4xl tracking-tighter leading-none text-right shrink-0 text-right">{r.collected || 0} / {r.goal || 150} <span className="text-base md:text-lg opacity-40 font-black leading-none shrink-0 text-right">د.أ</span></p><button onClick={() => onDonate(r, 'road')} className="bg-slate-900 text-white px-8 md:px-12 py-3 md:py-4 rounded-2xl md:rounded-3xl font-black text-lg md:text-xl hover:bg-green-600 transition-all active:scale-95 leading-none italic uppercase transition-all shrink-0 text-center text-right">تبرع</button></div></div></div>)) : <div className="col-span-full py-20 text-center text-slate-300 font-bold italic border-2 border-dashed border-slate-50 rounded-[2rem] text-right">لا توجد بلاغات نشطة حالياً.</div>}
    </div>
  </div>
);
...
const LeaderboardView = ({ donations, onBack }) => {
  ...
  return (
    <div className="py-8 md:py-12 px-2 space-y-10 animate-in slide-in-from-left text-right leading-none text-right">
      <div className="flex items-center gap-4 md:gap-6 text-right justify-end text-right"><div className="text-right text-right text-right"><h2 className="text-3xl md:text-5xl font-black text-slate-800 tracking-tighter italic uppercase text-right leading-none shrink-0 text-right">لوحة الشرف</h2><p className="text-slate-400 font-bold text-[10px] md:text-sm uppercase tracking-[0.2em] mt-2 italic text-right leading-none text-right">Elite National Contributors</p></div><button onClick={onBack} className="p-4 bg-white rounded-2xl border hover:bg-slate-50 active:scale-90 shadow-sm leading-none shrink-0 text-right"><ChevronLeft className="rotate-180 text-right" size={28}/></button></div>
      <div className="bg-white rounded-[2.5rem] md:rounded-[5rem] border border-slate-100 shadow-2xl overflow-hidden ring-1 ring-slate-100 text-right">
        {topDonors.map((d, i) => (
          <div key={i} className="p-8 md:p-14 flex items-center justify-between border-b border-slate-50 last:border-0 hover:bg-amber-50/20 transition-all group text-right leading-none text-right">
            <p className="text-green-600 font-black text-3xl md:text-5xl tracking-tighter leading-none text-left shrink-0 text-left">{d.total} <span className="text-lg md:text-xl opacity-60 font-bold leading-none text-left">د.أ</span></p>
            <div className="flex items-center gap-6 md:gap-10 text-right leading-none justify-end text-right">
               <div className="text-right leading-none text-right"><p className="font-black text-xl md:text-3xl text-slate-800 leading-none mb-2 text-right text-right">{d.name}</p><div className="flex items-center gap-2 text-green-600 text-[10px] md:text-xs font-black uppercase tracking-widest leading-none text-right justify-end text-right"><UserCheck size={16} className="text-right"/> مساهم وطني موثق</div></div>
...
const PartnerPortalView = ({ onBack }) => (
  <div className="py-8 md:py-12 space-y-10 animate-in fade-in text-right leading-none text-right">
     <div className="flex items-center gap-6 leading-none text-right justify-end text-right"><h2 className="text-3xl md:text-4xl font-black text-slate-800 tracking-tighter italic uppercase text-right leading-none shrink-0 text-right">بوابة الشركاء</h2><button onClick={onBack} className="p-4 bg-white rounded-2xl border hover:bg-slate-50 active:scale-90 shadow-sm transition-all leading-none shrink-0 text-right"><ChevronLeft className="rotate-180 text-right" size={28}/></button></div>
...
const NavBtn = ({ icon, label, active, onClick }) => (
  <button onClick={onClick} className={`flex flex-col items-center gap-1 transition-all active:scale-90 leading-none shrink-0 ${active ? 'text-green-500' : 'text-slate-500 hover:text-white'} text-right`}>
    <div className={`p-2 md:p-3.5 rounded-xl md:rounded-2xl transition-all shrink-0 ${active ? 'bg-green-500/10 shadow-inner' : ''} text-right`}>
       {React.cloneElement(icon, { size: 20, className: "md:w-[26px] md:h-[26px] text-right", strokeWidth: active ? 3 : 2.5 })}
    </div>
    <span className={`text-[7px] md:text-[10px] font-black uppercase tracking-widest transition-opacity leading-none shrink-0 ${active ? 'opacity-100' : 'opacity-40'} text-right`}>{label}</span>
  </button>
);
...
const LoadingScreen = () => (
  <div className="h-screen flex flex-col items-center justify-center bg-[#F8FAFC] gap-6 animate-in fade-in text-center px-10 leading-none text-right">
    <div className="relative leading-none text-right">
    ...
    <div className="space-y-2 leading-none text-right">
       ...
       <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest leading-none text-right">المنصة الوطنية الأردنية</p>
    </div>
  </div>
);
...
const InfoPopup = ({ item, onClose }) => (
  <div className="fixed inset-0 bg-slate-900/95 backdrop-blur-3xl z-[100] flex items-center justify-center p-4 md:p-6 animate-in fade-in leading-none text-right">
    <div className="bg-white rounded-[3rem] md:rounded-[6rem] w-full max-w-4xl p-8 md:p-20 space-y-10 shadow-2xl relative overflow-hidden animate-in zoom-in ring-1 ring-slate-100 text-right leading-none text-right">
      <button onClick={onClose} className="absolute top-6 right-6 md:top-12 md:right-12 p-4 bg-white/50 backdrop-blur-md rounded-full text-slate-800 z-20 border border-slate-200 shadow-xl active:scale-90 leading-none text-right"><X size={32} className="leading-none text-right"/></button>
      <div className="relative group text-right leading-none text-right">
        <img src={item.img} className="w-full h-[300px] md:h-[550px] rounded-[2.5rem] md:rounded-[5rem] object-cover shadow-2xl transition-transform duration-[3s] group-hover:scale-105 leading-none text-right" alt="Detail" />
        ...
        <div className="absolute bottom-6 right-6 md:bottom-12 md:right-12 leading-none text-right text-right">
          <p className="text-slate-200 font-bold text-base md:text-lg mb-4 flex items-center gap-3 justify-end uppercase tracking-[0.3em] leading-none italic text-right leading-none text-right text-right"><MapPin size={24} className="text-green-500 leading-none text-right"/> {item.location}</p>
      ...
      <div className="bg-slate-50 p-8 md:p-12 rounded-[2.5rem] md:rounded-[4.5rem] border border-slate-100 shadow-inner space-y-8 text-right leading-relaxed text-right text-right">
         <div className="inline-flex items-center gap-3 bg-indigo-600 text-white px-6 py-2 md:px-8 md:py-3 rounded-full font-black text-base md:text-lg shadow-xl leading-none italic text-right text-right leading-none text-right"><ShieldCheck size={24} className="leading-none text-right"/> تقرير هندسي معتمد</div>
         <p className="text-slate-600 text-xl md:text-3xl leading-[1.6] font-medium tracking-tight text-right leading-relaxed italic text-right text-right">يخضع هذا الموقع لرقابة مهندسي المساحة والتعبيد في أمانة عمان الكبرى. يتم تنفيذ الأعمال وفق كودات الطرق الأردنية المستدامة لضمان جودة التعبيد لأكثر من 15 عاماً ومقاومة كافة الظروف الجوية القاسية.</p>
      </div>
    </div>
  </div>
);
...
const DonationPopup = ({ item, onClose, onConfirm }) => {
  ...
  return (
    <div className="fixed inset-0 bg-slate-900/90 backdrop-blur-2xl z-[100] flex items-end md:items-center justify-center p-4 md:p-6 animate-in fade-in duration-300 leading-none text-right">
      <div className="bg-white rounded-t-[3rem] md:rounded-[4rem] w-full max-w-lg p-8 md:p-16 space-y-8 shadow-2xl animate-in slide-in-from-bottom-20 md:zoom-in-95 text-right leading-none text-right text-right">
        <div className="flex justify-between items-center leading-none text-right w-full leading-none text-right text-right text-right"><h3 className="text-3xl md:text-4xl font-black text-slate-800 tracking-tighter italic uppercase text-right leading-none text-right text-right">ساهم في البناء</h3><button onClick={onClose} className="p-3 bg-slate-100 rounded-full shadow-sm active:scale-90 leading-none text-right text-right"><X size={20} className="leading-none text-right text-right"/></button></div>
        <div className="bg-green-50 p-6 md:p-10 rounded-[2rem] border border-green-100 shadow-inner text-center leading-none text-right text-right text-right"><p className="font-black text-2xl md:text-3xl leading-[1.1] tracking-tight italic text-center leading-none text-right text-right">{item.item.location}</p></div>
        <div className="space-y-4 md:space-y-6 leading-none text-right text-right text-right text-right"><label className="text-xs font-black text-slate-400 mr-2 uppercase tracking-[0.3em] opacity-70 leading-none block italic text-right w-full leading-none text-right text-right">Contribution (JOD)</label><input type="number" value={amount} onChange={e => setAmount(e.target.value)} className="w-full p-8 md:p-10 bg-slate-50 border-2 border-transparent rounded-[2rem] md:rounded-[3rem] font-black text-center text-4xl md:text-6xl outline-none focus:border-green-600 focus:bg-white transition-all shadow-inner text-slate-900 italic leading-none text-right text-right" /></div>
        <div className="space-y-6 md:space-y-8 text-right leading-none text-right text-right text-right text-right text-right"><input placeholder="الاسم الكامل" disabled={anon} className="w-full p-6 md:p-8 bg-white border-2 border-slate-100 rounded-[1.8rem] md:rounded-[2.5rem] font-black outline-none focus:border-green-600 disabled:bg-slate-50 transition-all text-xl md:text-2xl shadow-sm text-right leading-none text-right text-right text-right text-right" value={name} onChange={e => setName(e.target.value)} /><label className="flex items-center gap-4 md:gap-6 cursor-pointer group justify-end leading-none text-right text-right text-right text-right"><span className="text-xl md:text-2xl font-black text-slate-500 group-hover:text-slate-800 transition-colors italic leading-none text-right text-right text-right text-right text-right">التبرع بهوية مجهولة</span><input type="checkbox" className="hidden" checked={anon} onChange={e => setAnon(e.target.checked)} /><div className={`w-8 h-8 md:w-10 md:h-10 rounded-xl md:rounded-2xl border-4 flex items-center justify-center transition-all leading-none text-right text-right ${anon ? 'bg-green-600 border-green-600 shadow-xl' : 'bg-white border-slate-200'}`}>{anon && <CheckCircle2 size={24} className="text-white leading-none text-right text-right" />}</div></label></div>
        <button onClick={() => onConfirm({ amount, donorName: name, anonymousName: anon, itemId: item.item.id, type: item.type })} className="w-full bg-amber-500 text-white py-8 md:py-10 rounded-[2rem] md:rounded-[3rem] font-black text-3xl md:text-4xl shadow-xl shadow-amber-200 active:scale-95 transition-all flex items-center justify-center gap-4 group leading-none italic uppercase text-center text-right leading-none text-right">تأكيد المساهمة <CheckCircle size={28} className="group-hover:scale-110 transition-transform leading-none text-right" /></button>
      </div>
    </div>
  );
};
