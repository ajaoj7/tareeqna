      ...
      <header className="bg-white/80 backdrop-blur-xl border-b border-slate-100 sticky top-0 z-[60] p-3 md:p-4 flex justify-between items-center shadow-sm">
        <div className="flex items-center gap-2 md:gap-3 cursor-pointer group" onClick={() => setView('landing')}>
          ...
          <div className="leading-none whitespace-nowrap">
            <h1 className="text-xl md:text-3xl font-black text-slate-900 tracking-tighter italic leading-none">
              طريق<span className="text-green-600 leading-none">نا</span>
            </h1>
        ...
        <div className="flex items-center gap-2 md:gap-4 leading-none">
          <button onClick={() => setView('leaderboard')} className="p-2 md:p-2.5 bg-amber-50 text-amber-600 rounded-xl border border-amber-100 active:scale-95 transition-all leading-none"><Trophy size={20}/></button>
        </div>
      </header>
...
// --- Landing View (Layout with Left Sidebar) ---
      ...
      <div className="flex flex-col lg:flex-row gap-8">
        ...
        <div className="flex-1 space-y-12 md:space-y-20">
          ...
          <section className="bg-slate-900 rounded-[2.5rem] md:rounded-[3.5rem] p-8 md:p-16 text-white relative overflow-hidden shadow-2xl border border-white/5">
            <div className="relative z-10 max-w-2xl space-y-6 md:space-y-8 text-right">
              <div className="inline-flex items-center gap-2 bg-green-500/10 text-green-400 px-3 py-1.5 md:px-4 md:py-2 rounded-full border border-green-500/20 text-[10px] md:text-xs font-black uppercase tracking-widest leading-none">
        ...
        <aside className="w-full lg:w-72 space-y-8">
           <div className="bg-white p-8 rounded-[3rem] border border-slate-100 shadow-sm sticky top-28">
              ...
              <div className="space-y-6">
                 {sponsors.map(s => (
                   <div key={s.id} className="flex items-center gap-4 group cursor-pointer">
                      <div className="w-12 h-12 rounded-2xl bg-slate-50 flex items-center justify-center p-2 border border-slate-50 group-hover:border-green-100 transition-all">
                      ...
                      <div className="flex-1 min-w-0">
                         ...
                         <p className="text-[10px] font-bold text-green-600">{s.repairs} مشروع منجز</p>
                      </div>
                   </div>
                 ))}
              </div>
              ...
              <button 
                onClick={() => setView('partner-portal')}
                className="w-full mt-10 bg-indigo-50 text-indigo-600 py-4 rounded-2xl font-black text-sm flex items-center justify-center gap-2 hover:bg-indigo-600 hover:text-white transition-all shadow-sm shadow-indigo-100 active:scale-95"
              >
                 <Lock size={14}/> بوابة الشركاء
              </button>
           </div>
           ...
           <div className="bg-slate-900 p-6 rounded-[2rem] text-white space-y-3">
              ...
              <p className="font-black text-xs">توثيق ذكي 100%</p>
  ...
  <div className="space-y-6 md:space-y-8 px-2">
    <div className="flex justify-between items-center border-r-4 border-slate-900 pr-4 leading-none">
       <div className="flex items-center gap-3">
          {icon}
          <h3 className="text-xl md:text-2xl font-black text-slate-800 leading-none">{title}</h3>
       </div>
...
const CameraReportView = ({ onComplete, user }) => {
  ...
  const openCamera = () => { if (fileInputRef.current) fileInputRef.current.click(); };
  ...
  const handleAIAnalysis = async () => {
    ...
    setTimeout(async () => {
      ...
      let streetType = "شارع فرعي"; let severity = 5; let goal = 150;

      if (locationLow.includes("جاردنز") || locationLow.includes("جامعة") || locationLow.includes("رينبو") || locationLow.includes("وصفي التل")) {
        streetType = "شارع حيوي وتجاري"; severity = 9; goal = 400;
      } else if (locationLow.includes("اتوستراد") || locationLow.includes("عام") || locationLow.includes("رئيسي")) {
        streetType = "طريق سريع / دولي"; severity = 8; goal = 300;
      }

      try {
        await addDoc(collection(db, 'artifacts', appId, 'public', 'data', 'reports'), {
          userId: user?.uid || "guest", img, location, streetType, severity, goal, collected: 0,
          createdAt: new Date().toISOString(), status: 'verified'
        });
        onComplete();
      } catch (e) { setError("فشل في الاتصال، حاول مجدداً."); setSubmitting(false); }
    }, 2000);
  };
  ...
  return (
    <div className="p-4 md:p-10 space-y-8 animate-in slide-in-from-bottom-12 max-w-4xl mx-auto text-right">
      ...
      {isPC && !img ? (
        ...
        <div className="bg-white p-8 md:p-10 rounded-[3rem] border-4 border-dashed border-slate-200 flex flex-col items-center justify-center min-h-[350px] relative overflow-hidden cursor-pointer group hover:border-green-500 shadow-inner transition-all" onClick={openCamera}>
          {img ? <img src={img} className="absolute inset-0 w-full h-full object-cover" alt="Capture" /> : 
            <div className="text-center space-y-6 animate-in zoom-in">
               ...
               <div><p className="text-2xl font-black text-slate-700 leading-none">إضغط للتصوير الميداني</p><p className="text-sm font-bold text-slate-400 mt-4 max-w-xs mx-auto text-center leading-relaxed italic">يجب تصوير الحفرة بوضوح لتقييم عمقها وخطورتها آلياً.</p></div>
            </div>
          }
         ...
         <div className="space-y-4">
            ...
            {error && (
              <div className="flex items-center gap-3 bg-red-100 text-red-600 p-4 md:p-5 rounded-[1.5rem] border-2 border-red-200 animate-in slide-in-from-top-4">
                <AlertCircle size={20}/>
                <p className="font-black text-base md:text-lg leading-snug">ملاحظة: يرجى كتابة عنوان الشارع بوضوح لسهولة وصول فرق الصيانة.</p>
              </div>
            )}
         </div>
      </div>

      <button onClick={handleAIAnalysis} disabled={submitting} className="w-full bg-slate-900 text-white py-6 md:py-8 rounded-[2rem] font-black text-2xl md:text-4xl shadow-2xl active:scale-95 disabled:bg-slate-400 transition-all flex items-center justify-center gap-4 group">
...
const ListView = ({ title, items, onDonate, onInfo }) => (
  <div className="py-8 md:py-12 space-y-10 animate-in slide-in-from-right duration-700 text-right">
     ...
     <div className="grid gap-8">
        ...
        )) : <div className="py-20 text-center space-y-8 bg-white rounded-[2.5rem] border-2 border-dashed border-slate-100"><AlertTriangle className="mx-auto text-slate-200 animate-pulse" size={80}/><p className="text-2xl font-black text-slate-300 tracking-tighter uppercase">لا يوجد مشاريع نشطة حالياً</p></div>}
     </div>
  </div>
);

const DetailedCard = ({ item, onDonate, onInfo }) => (
  <div className="bg-white p-6 md:p-12 rounded-[2.5rem] md:rounded-[4.5rem] border border-slate-100 shadow-sm space-y-8 md:space-y-10 group hover:shadow-2xl transition-all duration-700 text-right">
     <div className="flex flex-col lg:flex-row gap-8 md:gap-12">
        <div className="relative w-full lg:w-80 h-64 md:h-80 flex-shrink-0 overflow-hidden rounded-[2.5rem] md:rounded-[3.5rem] shadow-inner">
           ...
           <div className="absolute top-4 right-4 bg-red-600 text-white text-[9px] md:text-[10px] font-black px-4 py-1.5 md:px-6 md:py-2 rounded-full shadow-xl ring-2 md:ring-4 ring-white uppercase animate-pulse"> عاجل </div>
        </div>
        <div className="flex-1 space-y-6 md:space-y-8 text-right">
           <div className="flex justify-between items-start leading-none w-full">
              ...
              <button onClick={() => onInfo(item)} className="p-3 md:p-4 bg-indigo-50 text-indigo-600 rounded-xl md:rounded-[1.5rem] shadow-lg active:scale-90 hover:bg-indigo-600 hover:text-white transition-all leading-none"><Info size={24}/></button>
           </div>
    ...
    <div className="bg-white rounded-[3rem] md:rounded-[6rem] w-full max-w-4xl p-8 md:p-20 space-y-10 md:space-y-14 shadow-2xl relative overflow-hidden animate-in zoom-in ring-1 ring-slate-100 text-right leading-none">
      ...
      <div className="relative group text-right">
        ...
        <div className="absolute bottom-6 right-6 md:bottom-12 md:right-12 leading-none text-right">
          ...
          <h3 className="text-3xl md:text-6xl font-black text-white leading-tight tracking-tighter uppercase italic leading-none text-right">التفاصيل الفنية</h3>
        </div>
      </div>
      <div className="bg-slate-50 p-8 md:p-12 rounded-[2.5rem] md:rounded-[4.5rem] border border-slate-100 shadow-inner space-y-6 md:space-y-8 text-right leading-relaxed text-right">
...
const LeaderboardView = ({ donations, onBack }) => {
  ...
  return (
    <div className="py-8 md:py-12 px-2 space-y-10 animate-in slide-in-from-left text-right leading-none">
      <div className="flex items-center gap-4 md:gap-6 text-right"><button onClick={onBack} className="p-4 bg-white rounded-2xl border hover:bg-slate-50 active:scale-90 shadow-sm leading-none"><ChevronLeft className="rotate-180" size={28}/></button><div><h2 className="text-3xl md:text-5xl font-black text-slate-800 tracking-tighter italic uppercase text-right leading-none">لوحة الشرف</h2><p className="text-slate-400 font-bold text-[10px] md:text-sm uppercase tracking-[0.2em] mt-2 italic text-right leading-none">Elite National Contributors</p></div></div>
      <div className="bg-white rounded-[2.5rem] md:rounded-[5rem] border border-slate-100 shadow-2xl overflow-hidden ring-1 ring-slate-100">
        {topDonors.map((d, i) => (
          <div key={i} className="p-8 md:p-14 flex items-center justify-between border-b border-slate-50 last:border-0 hover:bg-amber-50/20 transition-all group text-right">
            ...
            <p className="text-green-600 font-black text-3xl md:text-5xl tracking-tighter leading-none text-left">{d.total} <span className="text-lg md:text-xl opacity-60 font-bold leading-none">د.أ</span></p>
          </div>
        ))}
      </div>
    </div>
...
const DonateUnifiedView = ({ reports, onDonate }) => (
  <div className="py-8 md:py-12 space-y-12 animate-in fade-in text-right leading-none">
    <div className="bg-amber-50 p-8 md:p-16 rounded-[2.5rem] md:rounded-[4rem] border-2 border-amber-100 flex flex-col md:flex-row items-center gap-8 md:gap-14 shadow-inner relative overflow-hidden">
       ...
       <div className="space-y-4 md:space-y-6 relative z-10 text-right leading-none">
    ...
    <div className="grid grid-cols-1 md:grid-cols-2 gap-8 md:gap-10 text-right">
       {reports.map(r => (<div key={r.id} className="bg-white p-6 md:p-10 rounded-[2.5rem] md:rounded-[4rem] border border-slate-100 shadow-sm space-y-6 md:space-y-8 group hover:shadow-xl transition-all leading-none text-right flex flex-col h-full"><img src={r.img} className="w-full h-56 md:h-64 rounded-[2rem] md:rounded-[3rem] object-cover shadow-xl grayscale-[0.2] group-hover:grayscale-0 transition-all duration-500" alt="Repair" /><div className="space-y-6 text-right flex-1"><h4 className="font-black text-2xl md:text-3xl truncate text-slate-800 italic leading-none text-right">{r.location}</h4><div className="flex justify-between items-center bg-slate-50 p-6 md:p-8 rounded-[1.5rem] md:rounded-[2rem] border border-slate-100 shadow-inner text-right"><p className="text-green-600 font-black text-2xl md:text-4xl tracking-tighter leading-none text-right">{r.collected || 0} / {r.goal || 150} <span className="text-base md:text-lg opacity-40 font-black leading-none">د.أ</span></p><button onClick={() => onDonate(r, 'road')} className="bg-slate-900 text-white px-8 md:px-12 py-3 md:py-4 rounded-2xl md:rounded-3xl font-black text-lg md:text-xl hover:bg-green-600 transition-all active:scale-95 leading-none italic uppercase transition-all">تبرع</button></div></div></div>))}
    </div>
  </div>
);
