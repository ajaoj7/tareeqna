...
const CameraReportView = ({ onComplete, user }) => {
  ...
  return (
    <div className="p-6 md:p-10 space-y-12 animate-in slide-in-from-bottom-12 max-w-4xl mx-auto">
      <div className="flex items-center justify-between leading-none"><h2 className="text-4xl font-black text-slate-800 tracking-tighter">بلاغ ميداني مباشر</h2><button onClick={onComplete} className="p-3 bg-white rounded-full text-slate-400 border border-slate-100 shadow-sm"><X/></button></div>
        ...
        {topDonors.map((d, i) => (
          <div key={i} className="p-10 md:p-14 flex items-center justify-between border-b border-slate-50 last:border-0 hover:bg-amber-50/20 transition-all group">
            <div className="flex items-center gap-10">
               ...
               <div><p className="font-black text-3xl text-slate-800 group-hover:text-slate-900 transition-colors tracking-tight italic leading-none">{d.name}</p><div className="flex items-center gap-2 text-green-600 text-sm font-black uppercase mt-3 tracking-widest"><UserCheck size={18}/> مساهم وطني موثق</div></div>
            </div>
            <p className="text-green-600 font-black text-5xl tracking-tighter leading-none">{d.total} <span className="text-xl opacity-60 font-bold">د.أ</span></p>
          </div>
        ))}
      </div>
    </div>
...
const DonateUnifiedView = ({ reports, onDonate }) => (
  <div className="py-12 space-y-16 animate-in fade-in duration-1000 text-right leading-none">
    ...
    <div className="space-y-12">
       <div className="px-4 space-y-2 border-r-8 border-green-600 pr-8 leading-none"><h3 className="text-3xl font-black text-slate-800 uppercase leading-none">إعمار الشوارع</h3><p className="text-slate-400 font-bold text-xs uppercase tracking-widest mt-2 italic">Active National Projects</p></div>
       <div className="grid md:grid-cols-2 gap-10">{reports.map(r => (<div key={r.id} className="bg-white p-10 rounded-[4rem] border border-slate-100 shadow-sm space-y-8 group hover:shadow-xl transition-all leading-none"><img src={r.img} className="w-full h-64 rounded-[3rem] object-cover shadow-xl grayscale-[0.2] group-hover:grayscale-0 transition-all duration-500" /><div className="space-y-6"><h4 className="font-black text-3xl truncate text-slate-800 italic leading-none tracking-tight">{r.location}</h4><div className="flex justify-between items-center bg-slate-50 p-8 rounded-[2rem] border border-slate-100 shadow-inner"><p className="text-green-600 font-black text-4xl tracking-tighter leading-none">{r.collected || 0} / {r.goal} <span className="text-lg opacity-40 font-black">د.أ</span></p><button onClick={() => onDonate(r, 'road')} className="bg-slate-900 text-white px-12 py-4 rounded-3xl font-black text-xl hover:bg-green-600 transition-all active:scale-95 shadow-xl leading-none italic uppercase">تبرع الآن</button></div></div></div>))}</div>
    </div>
  </div>
);
