...
import { 
  ...
  UserCheck, User, CheckCircle, ArrowRight, AlertCircle, Smartphone, HardHat, Eye, BarChart3, Mail, Map as MapPinIcon, Facebook, Twitter, Instagram
} from 'lucide-react';
...
const appId = "tareeqna_official_v2";
...
try {
  ...
  auth = getAuth(app);
  db = getFirestore(app);
} catch (error) { console.warn("Firebase Init..."); }
...
const App = () => {
  ...
  // تحميل الخطوط والتنسيق برمجياً لضمان الاستقرار على Vercel
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    const unsubR = onSnapshot(path, (snap) => {
      ...
      <header className="bg-white/80 backdrop-blur-xl border-b border-slate-100 sticky top-0 z-[60] p-3 md:p-4 flex justify-between items-center shadow-sm">
        <div className="flex items-center gap-2 md:gap-3 cursor-pointer group" onClick={() => setView('landing')}>
          ...
          <div className="flex flex-col leading-none">
            <h1 className="text-xl md:text-3xl font-black text-slate-900 tracking-tighter italic flex items-center">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-12">
        ...
        <div className="space-y-6">
          <div className="flex flex-col gap-2">
            <h4 className="text-xl font-black text-slate-900 leading-none">اتصل بنا</h4>
            <p className="text-xs text-slate-400 font-bold uppercase leading-none italic">Contact Us</p>
          </div>
          <div className="space-y-4">
            ...
            <div className="flex items-start gap-3 group cursor-pointer">
              ...
              <div className="space-y-1">
                ...
                <p className="text-[10px] text-slate-400 font-bold leading-tight italic">Wasfi El Tall Street, Amman, Jordan</p>
              </div>
            </div>
          </div>
        </div>

        {/* Column 2: Account/Dashboard */}
        <div className="space-y-6">
          <div className="flex flex-col gap-2">
            <h4 className="text-xl font-black text-slate-900 leading-none">بوابتي الرقمية</h4>
            <p className="text-xs text-slate-400 font-bold uppercase leading-none italic">My Account</p>
          </div>
        ...
        <div className="space-y-6">
          <div className="flex flex-col gap-2">
            <h4 className="text-xl font-black text-slate-900 leading-none">حول المنصة</h4>
            <p className="text-xs text-slate-400 font-bold uppercase leading-none italic">Our Services</p>
          </div>
          <ul className="space-y-3">
            <li onClick={() => setView('partner-portal')} className="group flex items-center gap-2 cursor-pointer">
               <div className="w-1.5 h-1.5 bg-slate-200 rounded-full group-hover:bg-indigo-500 transition-all"></div>
               <span className="text-sm font-black text-slate-500 group-hover:text-slate-900 transition-colors">عن "طريقنا"</span>
            </li>
            <li className="group flex items-center gap-2 cursor-pointer">
               <div className="w-1.5 h-1.5 bg-slate-200 rounded-full group-hover:bg-indigo-500 transition-all"></div>
               <span className="text-sm font-black text-slate-500 group-hover:text-slate-900 transition-colors">سياسة الخصوصية</span>
            </li>
            <li className="group flex items-center gap-2 cursor-pointer">
               <div className="w-1.5 h-1.5 bg-slate-200 rounded-full group-hover:bg-indigo-500 transition-all"></div>
               <span className="text-sm font-black text-slate-500 group-hover:text-slate-900 transition-colors">الشروط والأحكام</span>
            </li>
          </ul>
          <div className="flex gap-4 pt-4">
             {[Facebook, Twitter, Instagram].map((Icon, i) => (
               <button key={i} className="p-2.5 bg-slate-50 text-slate-400 rounded-xl hover:bg-slate-900 hover:text-white transition-all shadow-sm">
                 <Icon size={18} />
               </button>
             ))}
          </div>
        ...
        {/* Column 4: The Smart Verification Card (Relocated) */}
        <div className="lg:pr-8">
           <div className="bg-green-600 p-8 rounded-[3rem] text-white space-y-4 shadow-2xl relative overflow-hidden group hover:-translate-y-2 transition-transform duration-500">
              <div className="relative z-10">
                 <div className="flex items-center gap-4 mb-4 leading-none">
                    <div className="p-3 bg-white/20 rounded-2xl backdrop-blur-sm shadow-xl"><ShieldCheck className="text-white animate-pulse" size={32}/></div>
                    <div className="space-y-1">
                      <p className="font-black text-base uppercase italic leading-none">توثيق ذكي 100%</p>
                      <p className="text-[8px] font-bold text-green-100 uppercase tracking-widest leading-none">AI Identity Verification</p>
                    </div>
                 </div>
                 <p className="text-sm text-green-50 leading-relaxed font-bold italic">تخضع جميع البلاغات لنظام فحص الموقع والصورة عبر الذكاء الاصطناعي لضمان توجيه الدعم لمستحقيه فعلياً وبمصداقية وطنية.</p>
              </div>
              <div className="absolute -bottom-10 -left-10 w-40 h-40 bg-white/10 rounded-full blur-3xl group-hover:scale-150 transition-transform"></div>
              <div className="absolute top-0 right-0 w-20 h-2 bg-white/30 rounded-full mt-4 mr-4"></div>
           </div>
        </div>
...
// --- Landing View with Clean Sidebar ---
              ...
              <div className="space-y-6">
                 {sponsors.map(s => (
                   <div key={s.id} className="flex items-center gap-4 group cursor-pointer text-right transition-transform hover:-translate-x-1">
                      <div className="w-10 h-10 md:w-12 md:h-12 rounded-2xl bg-slate-50 flex items-center justify-center p-2 border border-slate-50 group-hover:border-green-100 group-hover:bg-white transition-all shadow-sm">
                         <img src={s.logo} className="w-full h-full object-contain grayscale group-hover:grayscale-0 transition-all" alt={s.name} />
                      </div>
                      <div className="flex-1 min-w-0">
                         <p className="font-black text-xs md:text-sm text-slate-800 truncate leading-none mb-1">{s.name}</p>
                         <p className="text-[9px] md:text-[10px] font-bold text-green-600 leading-none flex items-center gap-1">
                            <CheckCircle2 size={10}/> {s.repairs} مشروع منجز
                         </p>
                      </div>
                   </div>
                 ))}
      ...
      {items.length > 0 ? items.map(item => (
        <div key={item.id} onClick={() => setView('roads')} className="bg-white p-4 rounded-[2rem] border border-slate-100 shadow-sm hover:shadow-xl transition-all cursor-pointer group relative overflow-hidden flex flex-col h-full">
           <div className="relative h-40 md:h-48 overflow-hidden rounded-2xl mb-4">
              ...
              {isSuccess && <div className="absolute inset-0 bg-green-600/30 backdrop-blur-[2px] flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity"><div className="bg-white text-green-600 p-3 rounded-full shadow-2xl scale-0 group-hover:scale-100 transition-transform duration-500"><CheckCircle2 size={32}/></div></div>}
           </div>
    ...
    setTimeout(async () => {
      try {
        await addDoc(collection(db, 'artifacts', appId, 'public', 'data', 'reports'), {
          ...
          img,
          location,
          streetType: location.includes("مطار") || location.includes("اتوستراد") ? "طريق سيادي" : "شارع حيوي",
          severity: 8,
          goal: 400,
          pitDepth: (Math.random() * 10 + 5).toFixed(1),
          collected: 0,
        ...
        setError("تعذر الإرسال. تأكد من إعدادات Firestore.");
        setSubmitting(false);
      }
    }, 2000);
  };
  ...
  return (
    <div className="p-4 md:p-10 space-y-8 animate-in slide-in-from-bottom-12 max-w-4xl mx-auto text-right leading-none">
      <div className="flex items-center justify-between">
         <h2 className="text-3xl font-black text-slate-800 tracking-tighter leading-none">بلاغ ميداني ذكي</h2>
         <button onClick={onComplete} className="p-2 md:p-3 bg-white rounded-full text-slate-400 border border-slate-100 shadow-sm leading-none"><X/></button>
      </div>
      ...
      {isPC && !img ? (
        <div className="bg-amber-50 p-8 rounded-[2.5rem] border-2 border-amber-100 text-center space-y-6 animate-in zoom-in leading-relaxed">
           ...
           <h3 className="text-2xl font-black text-amber-900 leading-none italic uppercase">التصوير عبر الهاتف</h3>
           <p className="text-amber-800 font-bold text-base md:text-lg max-w-md mx-auto leading-relaxed italic">لضمان مصداقية البلاغ، يفضل تصوير الضرر مباشرة باستخدام هاتفك المحمول في الميدان.</p>
           <button onClick={() => fileInputRef.current.click()} className="text-amber-600 underline font-black leading-none">أو إرفاق صورة حالية</button>
        </div>
      ) : (
        <div className="bg-white p-8 md:p-10 rounded-[3rem] border-4 border-dashed border-slate-200 flex flex-col items-center justify-center min-h-[350px] relative overflow-hidden cursor-pointer group hover:border-green-500 shadow-inner transition-all" onClick={() => fileInputRef.current.click()}>
          {img ? <img src={img} className="absolute inset-0 w-full h-full object-cover" alt="Damage" /> : 
            <div className="text-center space-y-6 animate-in zoom-in leading-none">
               ...
               <div><p className="text-2xl font-black text-slate-700 leading-none italic uppercase">إضغط للتصوير</p><p className="text-sm font-bold text-slate-400 mt-4 max-w-xs mx-auto italic leading-none">سيتم تحليل الصورة آلياً.</p></div>
            </div>
          }
      ...
      <div className="bg-white p-8 rounded-[2.5rem] border border-slate-100 shadow-sm space-y-6 relative text-right leading-none">
         <div className="space-y-4 leading-none text-right">
            <label className={`text-xs font-black uppercase tracking-widest flex items-center gap-2 leading-none justify-end ${error ? 'text-red-600 animate-pulse' : 'text-slate-400'}`}>
            ...
            <div className="relative text-right">
              ...
              <input 
                placeholder="اكتب اسم الشارع أو أقرب معلم..." 
      ...
      <button onClick={handleSmartSubmission} disabled={submitting} className="w-full bg-slate-900 text-white py-6 md:py-8 rounded-[2rem] font-black text-2xl md:text-4xl shadow-2xl active:scale-95 disabled:bg-slate-400 transition-all flex items-center justify-center gap-4 group leading-none">
...
const ListView = ({ title, items, onDonate, onInfo, isStadium }) => (
  <div className="py-8 md:py-12 space-y-10 animate-in slide-in-from-right duration-700 text-right leading-none">
     <div className="px-2 border-r-8 border-slate-900 pr-4 md:pr-6 text-right leading-none">
        ...
        <p className="text-slate-400 font-bold text-[10px] md:text-xs uppercase tracking-widest mt-2 leading-none">Verified Infrastructure Hub</p>
     </div>
        ...
        {items.length > 0 ? items.map(item => (
          ...
          <div className="py-20 md:py-32 text-center space-y-8 bg-white rounded-[3rem] md:rounded-[5rem] border-2 border-dashed border-slate-100 shadow-inner flex flex-col items-center">
             ...
             <div className="space-y-4">
                ...
                <p className="text-slate-400 font-bold text-xs md:text-sm max-w-sm mx-auto leading-relaxed italic">سيتم تحديث هذه القائمة فور توفر تقارير ميدانية معتمدة.</p>
             </div>
          </div>
        )}
     </div>
  ...
  <div className="bg-white p-6 md:p-12 rounded-[2.5rem] md:rounded-[4.5rem] border border-slate-100 shadow-sm space-y-8 group hover:shadow-2xl transition-all duration-700 text-right leading-none">
     <div className="flex flex-col lg:flex-row gap-8 md:gap-12 leading-none text-right">
        ...
        <div className="flex-1 space-y-6 md:space-y-8 text-right flex flex-col justify-center leading-none">
           <div className="flex justify-between items-start leading-none w-full text-right">
              ...
              <button onClick={() => onInfo(item)} className="p-3 md:p-4 bg-indigo-50 text-indigo-600 rounded-xl shadow-lg active:scale-90 hover:bg-indigo-600 hover:text-white transition-all leading-none"><Info size={24}/></button>
           </div>
     ...
     <button onClick={() => onDonate(item)} className="w-full bg-amber-500 text-white py-6 md:py-8 rounded-[1.8rem] md:rounded-[2.5rem] font-black text-2xl md:text-4xl shadow-xl hover:bg-amber-600 active:scale-[0.98] transition-all flex items-center justify-center gap-4 group leading-none uppercase italic">
    ...
    <div className="bg-white rounded-[3rem] md:rounded-[6rem] w-full max-w-4xl p-8 md:p-20 space-y-10 shadow-2xl relative overflow-hidden animate-in zoom-in ring-1 ring-slate-100 text-right leading-none">
      ...
      <div className="relative group text-right leading-none">
        ...
        <div className="absolute bottom-6 right-6 md:bottom-12 md:right-12 leading-none text-right">
          ...
          <h3 className="text-3xl md:text-6xl font-black text-white leading-tight tracking-tighter uppercase italic text-right leading-none">التفاصيل الفنية</h3>
        </div>
      </div>
      ...
      <div className="bg-white rounded-[2.5rem] md:rounded-[5rem] border border-slate-100 shadow-2xl overflow-hidden ring-1 ring-slate-100">
        {topDonors.map((d, i) => (
          <div key={i} className="p-8 md:p-14 flex items-center justify-between border-b border-slate-50 last:border-0 hover:bg-amber-50/20 transition-all group text-right leading-none">
            <div className="flex items-center gap-6 md:gap-10 text-right leading-none">
...
const DonateUnifiedView = ({ reports, onDonate }) => (
  <div className="py-8 md:py-12 space-y-12 animate-in fade-in text-right leading-none">
    <div className="bg-amber-50 p-8 md:p-16 rounded-[2.5rem] md:rounded-[4rem] border-2 border-amber-100 flex flex-col md:flex-row items-center gap-8 shadow-inner relative overflow-hidden text-right leading-none">
       <div className="p-8 bg-white rounded-[2rem] shadow-xl relative z-10 leading-none"><Wallet className="text-amber-600 md:w-16 md:h-16" size={50}/></div>
       <div className="space-y-4 md:space-y-6 relative z-10 text-right w-full leading-none">
...
const PartnerPortalView = ({ onBack }) => (
  <div className="py-8 md:py-12 space-y-10 animate-in fade-in text-right leading-none">
     <div className="flex items-center gap-6 leading-none text-right"><button onClick={onBack} className="p-4 bg-white rounded-2xl border hover:bg-slate-50 active:scale-90 shadow-sm transition-all leading-none shrink-0"><ChevronLeft className="rotate-180" size={28}/></button><h2 className="text-3xl md:text-4xl font-black text-slate-800 tracking-tighter italic uppercase text-right leading-none shrink-0">بوابة الشركاء</h2></div>
