...
import { 
  ...
  UserCheck, User, CheckCircle, ArrowRight, AlertCircle, Smartphone, HardHat, Eye, BarChart3
} from 'lucide-react';
...
// --- Landing View with Vertical Sidebar on the LEFT ---
const LandingView = ({ setView, reports, sponsors }) => {
  ...
  return (
    <div className="py-6 md:py-10 animate-in fade-in duration-1000">
      {/* Layout Grid: Sidebar LEFT, Main RIGHT in PC */}
      <div className="flex flex-col lg:flex-row gap-8">
        
        {/* SIDEBAR (Left Column on PC) */}
        <aside className="w-full lg:w-72 space-y-6 order-2 lg:order-1">
           {/* الشركاء قائمة عمودية */}
           ...
           {/* بطاقة التوثيق الذكي - تأتي أسفل الشركاء لضمان عدم التعارض */}
           <div className="bg-green-600 p-6 rounded-[2rem] text-white space-y-3 shadow-xl relative overflow-hidden group">
              <div className="relative z-10">
                 <div className="flex items-center gap-3 mb-2">
                    <ShieldCheck className="text-white animate-bounce" size={24}/>
                    <p className="font-black text-sm uppercase italic">توثيق ذكي 100%</p>
                 </div>
                 <p className="text-[10px] text-green-50 leading-relaxed font-bold">تخضع جميع البلاغات لنظام فحص الموقع والصورة عبر الذكاء الاصطناعي لضمان توجيه الدعم لمستحقيه فعلياً.</p>
              </div>
        ...
        <div className="flex-1 space-y-12 md:space-y-20 order-1 lg:order-2">
          ...
          <section className="bg-slate-900 rounded-[2.5rem] md:rounded-[3.5rem] p-8 md:p-16 text-white relative overflow-hidden shadow-2xl border border-white/5">
            <div className="relative z-10 max-w-2xl space-y-6 text-right">
              ...
              <p className="text-slate-400 text-sm md:text-xl font-medium leading-relaxed max-w-lg">المنصة الذكية الموثقة لإصلاح وتجميل البنية التحتية بالتعاون مع القطاع الخاص.</p>
...
const HomeSection = ({ title, items, setView, icon, isSuccess }) => (
  <div className="space-y-6 px-2">
    <div className="flex justify-between items-center border-r-4 border-slate-900 pr-4 leading-none text-right">
       ...
       <button onClick={() => setView('roads')} className="text-slate-400 font-bold text-xs hover:text-green-600 transition-colors">الكل</button>
    </div>
      ...
      {items.length > 0 ? items.map(item => (
        <div key={item.id} onClick={() => setView('roads')} className="bg-white p-4 rounded-[2rem] border border-slate-100 shadow-sm hover:shadow-xl transition-all cursor-pointer group relative overflow-hidden flex flex-col h-full">
           ...
           <div className="space-y-2 flex-1 text-right">
              <h4 className="font-black text-slate-800 text-sm md:text-lg truncate leading-tight italic">{item.location}</h4>
...
// --- Jordan Intelligence Engine & Camera View ---
const CameraReportView = ({ onComplete, user }) => {
  ...
  // --- محرك ذكاء الأردن الجغرافي ---
  const jordanMegaAI = (text) => {
    const txt = text.toLowerCase();
    const db = {
      sovereign: ["مطار", "اتوستراد", "مية", "بيرين", "جرش", "اربد", "ستوني", "دولي", "سريع", "صحراوي", "بحر ميت"],
      commercial: ["جاردنز", "جامعة", "رينبو", "عبدلي", "سويفية", "خلدا", "مكة", "مدينة", "وصفي", "تلاع", "صويفية", "رابية", "شميساني"],
      landmarks: ["مول", "مستشفى", "حدائق", "جامع", "كنيسة", "سياح", "اثر", "قلعة", "مدرج", "بوليفارد", "ستي مول", "مكة مول", "البوابة"],
    };

    if (db.sovereign.some(k => txt.includes(k))) return { type: "طريق سيادي / دولي", severity: 9, goal: 550, importance: "أمن قومي" };
  ...
  const handleSmartSubmission = async () => {
    ...
    // محاكاة تحليل AI للصورة والموقع
    setTimeout(async () => {
      const evaluation = jordanMegaAI(location);
      const calculatedDepth = (Math.random() * 12 + 4).toFixed(1); // عمق عشوائي يحاكي تحليل الصورة
    ...
    <div className="p-4 md:p-10 space-y-8 animate-in slide-in-from-bottom-12 max-w-4xl mx-auto text-right">
      ...
      {isPC && !img ? (
        <div className="bg-amber-50 p-8 rounded-[2.5rem] border-2 border-amber-100 text-center space-y-6 animate-in zoom-in">
           ...
           <p className="text-amber-800 font-bold text-base md:text-lg max-w-md mx-auto leading-relaxed">لضمان مصداقية البلاغ وتوثيق عمق الحفرة آلياً، يفضل تصوير الضرر مباشرة باستخدام هاتفك المحمول في الميدان. (يمنع استخدام صور الاستوديو)</p>
           <button onClick={() => fileInputRef.current.click()} className="text-amber-600 underline font-black">أو إضغط هنا لاستخدام الكاميرا المدمجة</button>
        </div>
      ) : (
        <div className="bg-white p-8 md:p-10 rounded-[3rem] border-4 border-dashed border-slate-200 flex flex-col items-center justify-center min-h-[350px] relative overflow-hidden cursor-pointer group hover:border-green-500 transition-all shadow-inner" onClick={() => fileInputRef.current.click()}>
          {img ? <img src={img} className="absolute inset-0 w-full h-full object-cover" alt="Verification" /> : 
            <div className="text-center space-y-6 animate-in zoom-in">
               ...
               <div><p className="text-2xl font-black text-slate-700 leading-none italic uppercase">إضغط للتصوير الميداني</p><p className="text-sm font-bold text-slate-400 mt-4 max-w-xs mx-auto italic">سيقوم الذكاء الاصطناعي بتحليل عمق الضرر تلقائياً.</p></div>
            </div>
          }
      ...
      <div className="bg-white p-8 rounded-[2.5rem] border border-slate-100 shadow-sm space-y-6 relative text-right">
         <div className="space-y-4">
            ...
            <div className="relative">
              ...
              <input 
                placeholder="مثلاً: شارع المطار، بعد جسر مادبا بـ 100 متر..." 
      ...
      <button onClick={handleSmartSubmission} disabled={submitting} className="w-full bg-slate-900 text-white py-6 md:py-8 rounded-[2rem] font-black text-2xl md:text-4xl shadow-2xl active:scale-95 disabled:bg-slate-400 transition-all flex items-center justify-center gap-4 group leading-none">
        {submitting ? <Loader2 className="animate-spin" size={32}/> : <>تحقق ونشر البلاغ الوطني <HardHat size={28} className="rotate-12 group-hover:rotate-0 transition-transform"/></>}
      </button>
    </div>
  );
};

// --- ListView with Stadium Support ---
const ListView = ({ title, items, onDonate, onInfo, isStadium }) => (
  <div className="py-8 md:py-12 space-y-10 animate-in slide-in-from-right duration-700 text-right">
     <div className="px-2 border-r-8 border-slate-900 pr-4 md:pr-6 text-right leading-none">
        ...
        <p className="text-slate-400 font-bold text-[10px] md:text-xs uppercase tracking-widest mt-2">Jordan Infrastructure Master Database</p>
     </div>
     <div className="grid gap-8">
        {items.length > 0 ? items.map(item => (
           <div key={item.id} className="bg-white p-6 md:p-12 rounded-[2.5rem] md:rounded-[4.5rem] border border-slate-100 shadow-sm space-y-8 group hover:shadow-2xl transition-all duration-700 text-right">
              <div className="flex flex-col lg:flex-row gap-8 md:gap-12">
                 <div className="relative w-full lg:w-80 h-64 md:h-80 flex-shrink-0 overflow-hidden rounded-[2.5rem] md:rounded-[3.5rem] shadow-inner">
                    ...
                    <div className={`absolute top-4 right-4 text-white text-[9px] md:text-[10px] font-black px-4 py-1.5 rounded-full shadow-xl ring-2 md:ring-4 ring-white uppercase animate-pulse ${item.severity >= 8 ? 'bg-red-600' : 'bg-green-600'}`}> {item.severity >= 8 ? 'خطر جداً' : 'أولوية قصوى'} </div>
                 ...
                 <div className="flex-1 space-y-6 md:space-y-8 text-right flex flex-col justify-center">
                    ...
                    <div className="flex flex-wrap gap-2 justify-end">
                       ...
                       <span className="bg-indigo-100 text-indigo-700 px-3 py-1 rounded-full text-[10px] font-black italic">أثر {item.importance}</span>
                    </div>
              ...
              <button onClick={() => onDonate(item)} className="w-full bg-amber-500 text-white py-6 md:py-8 rounded-[1.8rem] md:rounded-[2.5rem] font-black text-2xl md:text-4xl shadow-xl hover:bg-amber-600 active:scale-[0.98] transition-all flex items-center justify-center gap-4 group leading-none uppercase italic">
                <Heart fill="currentColor" size={28} className="group-hover:scale-125 transition-transform" /> ساهم في الإعمار الوطني
              </button>
           </div>
        )) : (
          <div className="py-20 md:py-32 text-center space-y-8 bg-white rounded-[3rem] md:rounded-[5rem] border-2 border-dashed border-slate-100 shadow-inner flex flex-col items-center">
             ...
             <div className="space-y-4">
                ...
                <p className="text-slate-400 font-bold text-xs md:text-sm max-w-sm mx-auto leading-relaxed">سيتم تحديث هذه القائمة فور توفر تقارير فنية معتمدة من اللجنة الهندسية للمنصة.</p>
             </div>
          </div>
        )}
     </div>
  ...
  <div className="py-8 md:py-12 space-y-12 animate-in fade-in text-right leading-none">
    <div className="bg-amber-50 p-8 md:p-16 rounded-[2.5rem] md:rounded-[4rem] border-2 border-amber-100 flex flex-col md:flex-row items-center gap-8 shadow-inner relative overflow-hidden text-right leading-none">
       ...
       <div className="space-y-4 md:space-y-6 relative z-10 text-right w-full leading-none">
         ...
         <p className="text-amber-700 text-lg md:text-2xl font-bold opacity-80 leading-relaxed italic text-right">تتبرعاتك تذهب مباشرة لصيانة شوارعنا وملاعبنا استعداداً للاستحقاقات الوطنية الكبرى. كل دينار يساهم في جعل وطننا أكثر أماناً وجمالاً.</p>
       </div>
    </div>
