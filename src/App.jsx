...
const LoadingScreen = () => (
  <div className="h-screen flex flex-col items-center justify-center bg-[#F8FAFC] gap-6 animate-in fade-in">
    ...
    <p className="text-2xl font-black text-slate-800 italic">طريقنا</p>
  </div>
);
      ...
      <div className="flex gap-6 overflow-x-auto scrollbar-hide pb-4">
         {sponsors.map(s => (
           <div key={s.id} className="min-w-[220px] bg-white p-8 rounded-[3rem] border border-slate-100 shadow-sm flex flex-col items-center gap-5 hover:shadow-xl hover:-translate-y-2 transition-all cursor-pointer group relative overflow-hidden">
              ...
              <div className="text-center space-y-1"><p className="font-black text-slate-800 text-lg tracking-tight">{s.name}</p><p className="text-[10px] text-green-600 font-black uppercase mt-1">{s.repairs} مشروع منجز</p></div>
           </div>
         ))}
      </div>
    </div>
...
const ListView = ({ title, items, onDonate, onInfo }) => (
  <div className="py-12 space-y-12 animate-in slide-in-from-right duration-700 text-right">
     ...
     <div className="grid gap-12 text-right">
        ...
        )) : <div className="py-32 text-center space-y-8 bg-white rounded-[5rem] border-2 border-dashed border-slate-100 shadow-inner"><AlertTriangle className="mx-auto text-slate-200 animate-pulse" size={100}/><p className="text-3xl font-black text-slate-300 tracking-tighter leading-none uppercase italic text-center">لا يوجد مشاريع نشطة حالياً</p></div>}
     </div>
  </div>
);
  ...
  <div className="py-12 space-y-16 animate-in fade-in duration-1000 text-right leading-none">
    <div className="bg-amber-50 p-10 md:p-16 rounded-[4rem] border-2 border-amber-100 flex flex-col md:flex-row items-center gap-14 shadow-inner relative overflow-hidden">
       ...
       <div className="space-y-6 relative z-10 max-w-2xl leading-relaxed text-right">
         <h2 className="text-4xl font-black text-amber-900 leading-none tracking-tighter italic uppercase text-right w-full">صندوق إعمار الأردن الرقمي</h2>
  ...
  <div className="fixed inset-0 bg-slate-900/95 backdrop-blur-3xl z-[100] flex items-center justify-center p-6 animate-in fade-in">
    ...
    0 space-y-14 shadow-2xl relative overflow-hidden animate-in zoom-in ring-1 ring-slate-100 text-right leading-none">
      ...
      <div className="relative group text-right">
        ...
        <div className="absolute bottom-12 right-12 left-12 leading-none text-right">
          <p className="text-slate-200 font-bold text-lg mb-4 flex items-center gap-3 justify-end uppercase tracking-[0.3em] leading-none italic text-right"><MapPin size={24} className="text-green-500"/> {item.location}</p>
