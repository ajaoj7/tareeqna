...
// --- Camera & AI Logic (Fixed Submission) ---
const CameraReportView = ({ onComplete, user }) => {
  ...
  // --- مكبس الصور الذكي: الحل الجذري لمشكلة الـ 1MB ---
  const compressImage = (base64) => {
    return new Promise((resolve) => {
      const img = new Image(); img.src = base64;
      img.onload = () => {
        const canvas = document.createElement('canvas');
        const MAX_WIDTH = 1000; // تقليل العرض لضمان حجم صغير
        let width = img.width;
  ...
  const handleSmartSubmission = async () => {
    ...
    const submissionTimeout = setTimeout(() => {
        if (submitting) {
            setSubmitting(false);
            setError("بطء في الاتصال، يرجى المحاولة لاحقاً.");
        }
    }, 25000);
    ...
    try {
      ...
      // تحليل وهمي لسرعة الرد
      const streetType = location.includes("مطار") || location.includes("اتوستراد") ? "طريق سيادي" : "شارع حيوي";

      await addDoc(collection(db, 'artifacts', appId, 'public', 'data', 'reports'), {
        userId: user?.uid || "guest",
        img: compressedImg, // الصورة المضغوطة
        location,
        streetType,
        severity: 8,
        goal: 400,
      ...
      clearTimeout(submissionTimeout);
      setError("حدث خطأ فني أثناء الإرسال. تأكد من الإنترنت.");
      setSubmitting(false);
    }
  };
  ...
  return (
    <div className="p-4 md:p-10 space-y-8 animate-in slide-in-from-bottom-12 max-w-4xl mx-auto text-right leading-none">
      <div className="flex items-center justify-between leading-none text-right"><h2 className="text-3xl font-black text-slate-800 tracking-tighter leading-none text-right">بلاغ ميداني ذكي</h2><button onClick={onComplete} className="p-2 md:p-3 bg-white rounded-full text-slate-400 border border-slate-100 shadow-sm leading-none"><X/></button></div>
      ...
      {isPC && !img ? (
        <div className="flex flex-col md:flex-row gap-8 items-center bg-amber-50 p-8 md:p-12 rounded-[2.5rem] border-2 border-amber-100 animate-in zoom-in leading-relaxed text-right">
           ...
           <div className="bg-white p-6 rounded-[2rem] shadow-xl border border-amber-200 flex flex-col items-center shrink-0">
              ...
              <p className="text-[10px] font-black text-slate-400 mt-4 uppercase tracking-widest text-center leading-none text-right">امسح الكود للإكمال</p>
           </div>
        </div>
      ) : (
